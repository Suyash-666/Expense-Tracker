...
import { getExpenses, getBudgets, createBudget, updateBudget, getIncomes, createIncome, deleteIncome } from '../services/api';
...
const Budget = () => {
  ...
  const [primaryIncomeId, setPrimaryIncomeId] = useState(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [expensesRes, budgetsRes, incomesRes] = await Promise.all([
          getExpenses(),
          getBudgets(),
          getIncomes(),
        ]);
        ...
        const overallBudget = budgetsRes.find((b) => b.category === 'overall' && b.month?.startsWith(monthKey));
        if (overallBudget) {
          setBudget(overallBudget.limit || 0);
          setBudgetInput(String(overallBudget.limit || 0));
          setBudgetId(overallBudget._id || overallBudget.id);
        } else {
          setBudget(0);
        }

        const primaryIncome = incomesRes.find((inc) => inc.description === 'primary-budget-income');
        if (primaryIncome) {
          setIncome(primaryIncome.amount || 0);
          setIncomeInput(String(primaryIncome.amount || 0));
          setPrimaryIncomeId(primaryIncome._id || primaryIncome.id);
        } else {
          const totalIncome = incomesRes.reduce((sum, inc) => sum + (inc.amount || 0), 0);
          setIncome(totalIncome);
          setIncomeInput(totalIncome ? String(totalIncome) : '');
        }
      } catch (error) {
  ...
  const handleBudgetSave = async () => {
    ...
    try {
      ...
      if (budgetId) {
        await updateBudget(budgetId, { limit: budgetValue, category: 'overall', month: monthKey });
      } else {
        const created = await createBudget({ limit: budgetValue, category: 'overall', month: monthKey });
        setBudgetId(created._id || created.id);
      }

      if (primaryIncomeId) {
        await deleteIncome(primaryIncomeId);
      }
      const incomeDoc = await createIncome({
        source: 'Primary Income',
        amount: incomeValue,
        date: new Date().toISOString(),
        description: 'primary-budget-income'
      });
      setPrimaryIncomeId(incomeDoc._id || incomeDoc.id);
