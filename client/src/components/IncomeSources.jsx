...
const IncomeSources = () => {
  ...
  const parseMeta = (description = '') => {
    const [category, freq, recurringFlag] = description.split('|');
    return {
      category: category || 'salary',
      frequency: freq || 'monthly',
      isRecurring: recurringFlag !== 'one-time'
    };
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createIncome({
        ...
        date: formData.date,
        description: `${formData.category}|${formData.frequency}|${formData.isRecurring ? 'recurring' : 'one-time'}`
      });
  ...
  const calculateMonthlyIncome = () => {
    return sources.reduce((total, source) => {
      const meta = parseMeta(source.description);
      if (!meta.isRecurring) return total;
  ...
  const totalOneTime = sources
    .filter((s) => !parseMeta(s.description).isRecurring)
